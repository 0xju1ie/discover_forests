...
function App() {
  ...
  function handleChange(value) {
     if(value === conservation || value === reforestation){
      axios.get(apiUrlBase + "forests/" + value)
      .then(res => {
        setFoundForests(res.data)
      })
     }
  ...
  return (
    <div data-testid="app" className="app">
          <Search className="search" data-testid="search" defaultValue='' placeholder="Search by forest name"  value={name} allowClear onChange={filter} />
          <Select className="filter" placeholder="Forest Type" onChange={handleChange}>
